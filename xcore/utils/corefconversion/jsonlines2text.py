...
def highlight_clusters(tokens, clusters, paragraphs, *, singleton_color, color_manager, add_indices):
    ...
    if paragraphs:
        hl = Highlighter(prefix="<p>", suffix="</p>")
    ...
    for i, cluster in enumerate(clusters, start=1):
        ...
        if len(cluster) == 1:
            if singleton_color == "":
                ...
                color = (cm.gray if cm else "gray") if singleton_color is None else singleton_color
                start_span = f'<span style="color: {color};">'
                end_span = "</span>"
                hl = Highlighter(prefix=f"{start_span}[{end_span}", suffix=f"{start_span}]{end_span}")
        else:
            color = cm.get_next_color() if cm else "black"
            start_span = f'<span style="color: {color};">'
            end_span = "</span>"
            index = f"<sub>{counter}</sub>{end_span}" if add_indices else ""
            hl = Highlighter(prefix=f"<b>{start_span}[{end_span}", suffix=f"{start_span}]{index}</b>")
            counter += 1
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        prog="jsonlines2text", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter
    )
    ...
    parser.add_argument(
        "--sing-color",
        dest="singleton_color",
        help="singleton color: COLOR (default is 'gray') or \"\" to hide " "singleton markers",
        default=None,
    ),
    parser.add_argument(
        "-i",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="use the 'clusters' key even if a 'predicted_clusters' key is " "present",
    )
    parser.add_argument("-n", dest="n", default=0, type=int, help="number of tokens to consider from the beginning of the text")
    parser.add_argument(
        "--heading",
        dest="heading",
        default="<h1>%s</h1>",
        help="template for text name, default is '<h1>%s</h1>'.  Leave " "blank to ignore doc name",
    )
    # reading
...
def main():
    ...
    for line in open(args.infpath):
        ...
        res += convert(
            ...
            color_manager=args.color_manager,
            add_indices=args.add_indices,
        )
    if args.outfpath:
