...
def ontonotes_to_dataframe(file_path):
    ...
    # add speakers
    if "speakers" in df.columns and "wkc" not in file_path and "f" not in file_path:
...
def temp_dataframe(file_path):
    ...
    if "clusters" in df.columns:
        df = df[["doc_key", "tokens", "speakers", "clusters", "EOS_indices", "sentence2doc"]]
    else:
        df = df[["doc_key", "tokens", "speakers", "EOS_indices", "sentence2doc"]]
    df = df.dropna()
    df = df.reset_index(drop=True)
    return df
...
def original_token_offsets3(clusters, subtoken_map, new_token_map):
    return [
        tuple(
            [
                (
                    new_token_map[subtoken_map[i + start]],
                    new_token_map[subtoken_map[i + end]],
                )
                for i, start, end in cluster
                if subtoken_map[start] is not None
                and subtoken_map[end] is not None  # only happens first evals, model predicts <s> as mentions
            ]
        )
...
def original_token_offsetst(mode, tempp, clusters, subtoken_maps, new_token_maps):
    if mode == "cross":
        offset_to_idx = {t:i for i,t in enumerate(tempp)}
        return [
            tuple(
                [
                    (   offset_to_idx[off],
                        new_token_maps[offset_to_idx[off]][subtoken_maps[offset_to_idx[off]][start]],
                        new_token_maps[offset_to_idx[off]][subtoken_maps[offset_to_idx[off]][end]],
                    )
                    for off, start, end in cluster
                    if subtoken_maps[offset_to_idx[off]][start] is not None
                    and subtoken_maps[offset_to_idx[off]][end] is not None  # only happens first evals, model predicts <s> as mentions
                ]
            )
            for cluster in clusters
        ]
        ...
        return [
            tuple(
                [
                    (
                        new_token_maps[subtoken_maps[i + start]],
                        new_token_maps[subtoken_maps[i + end]],
                    )
                    for i, start, end in cluster
                    if subtoken_maps[start] is not None
                    and subtoken_maps[end] is not None  # only happens first evals, model predicts <s> as mentions
                ]
            )
            for cluster in clusters
        ]
